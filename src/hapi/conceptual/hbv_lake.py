...
The ``hapi.conceptual.hbv_lake`` module provides the ``HBVLake`` class, an
...
class HBVLake(BaseConceptualModel):
    ...
    Examples:
        >>> import numpy as np
        >>> from hapi.conceptual.hbv_lake import HBVLake, DEF_ST, DEF_q0
        >>> model = HBVLake()
        >>> n = 10
        >>> prec = np.array([5.0] * n)
    ...
    def _step_run(self, parameters, catchment_parameters, v, state_variables, curve):
        ...
        Examples:
            >>> import numpy as np
            >>> from hapi.conceptual.hbv_lake import HBVLake, DEF_ST
            >>> model = HBVLake()
